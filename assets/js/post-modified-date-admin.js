(function($) {
    ...
    // Notice Hide
    $('body').on('click', '.jltpmd-upgrade-popup .popup-dismiss', function(evt) {
        evt.preventDefault();
        $(this).closest('.jltpmd-upgrade-popup').fadeOut(200);
    });

    // Notice Show
    $('body').on('click', '.disabled', function(evt) {
        evt.preventDefault();
        $('.jltpmd-upgrade-popup').fadeIn(200);
    });
    ...
    // Search
    $('.jltpmd-search-plugins #search-plugins').on('keyup', function() {
        var value = $(this).val();
...
function activatePlugin(element, file) {
    ...
    jQuery.ajax({
        url: JLTPMDCORE.admin_ajax,
        type: "POST",
        data: {
            action: "jltpmd_recommended_activate_plugin",
            file: file,
            nonce: JLTPMDCORE.recommended_nonce
        },
...
function installPlugin(element, plugin) {
    ...
    jQuery.ajax({
        ...
        type: "POST",
        data: {
            action: "jltpmd_recommended_upgrade_plugin",
            type: 'install',
            plugin: plugin,
            nonce: JLTPMDCORE.recommended_nonce
        },
