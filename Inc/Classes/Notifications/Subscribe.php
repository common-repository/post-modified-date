...
if ( ! class_exists( 'Subscribe' ) ) {
	...
	class Subscribe extends Notice {
		...
		public function __construct() {
			parent::__construct();
			add_action( 'wp_ajax_jltpmd_subscribe', array( $this, 'jltpmd_subscribe' ) );
		}
		 ...
		 * @author Jewel Theme <support@jeweltheme.com>
		 */
		public function jltpmd_subscribe() {
			check_ajax_referer( 'jltpmd_subscribe_nonce' );
		...
		public function notice_content() {
			...
			?>
			<div class="notice notice-jltpmd is-dismissible notice-plugin-review notice-<?php echo esc_attr( $this->color ); ?> jltpmd-notice-<?php echo esc_attr( $this->get_id() ); ?>">
				<button type="button" class="notice-dismiss jltpmd-notice-dismiss"></button>
				<img width="70" height="70" src="<?php echo esc_url( JLTPMD_IMAGES . '/logo.png' ); ?>" alt="<?php esc_attr_e( 'Logo', 'post-modified-date' ); ?>">
				<div class="jltpmd-subscribe-content">
					<?php $this->set_title(); ?>
					<form style="display:flex">
						<div class="jltpmd-plugin-subscribe-input">
						...
						<button type="submit" class="button button-primary jltpmd-subscribe-btn"><?php esc_html_e( 'Get Discount', 'post-modified-date' ); ?></button>
					</form>
				</div>
			</div>
			<?php
		...
		public function core_script( $trigger_time ) {
			...
			<script>
				jQuery(document).on('submit', '.jltpmd-notice-subscribe .jltpmd-subscribe-content form', function(e) {
					...
					let formWrapper = form.closest('.jltpmd-subscribe-content');
					...
					jQuery.ajax({
							...
							crossDomain: true,
							data: {
								action: 'jltpmd_subscribe',
								_wpnonce: '<?php echo esc_js( wp_create_nonce( 'jltpmd_subscribe_nonce' ) ); ?>',
								name: name,
								email: email,
							}
						})
						.done(function(response) {
							formWrapper.hide().after('<p class="jltpmd--notice-message"><strong>' + response.data + '</strong><p>');
							let subsTimeout = setTimeout(function() {
								jltpmd_notice_action(null, form, 'disable');
