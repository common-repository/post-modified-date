...
class Notifications {
	...
	public function __construct() {
		...
		add_action( 'jltpmd_display_notice', array( $this, 'display_notice' ), 10, 2 );
		add_action( 'jltpmd_display_popup', array( $this, 'display_popup' ), 10, 2 );

		add_action( 'wp_ajax_jltpmd_notification_action', array( $this, 'notification_action' ) );
	}
	...
	public function notification_action() {
		check_ajax_referer( 'jltpmd_notification_nonce' );
	...
	public function setup_notifications_by_type( $type ) {
		...
		// Block if necessary .
		$notification_last_fired = get_option( "jltpmd_{$type}_last_interact" );
		...
		do_action( "jltpmd_display_{$type}", $notification, $trigger_time );
	}
	...
	public function display_popup( $popup, $trigger_time ) {
		...
		<div class="jltpmd-popup" id="jltpmd-popup" data-plugin="<?php echo esc_attr( $this->slug ); ?>" tabindex="1">

			<div class="jltpmd-popup-overlay"></div>

			<div class="jltpmd-popup-modal" style="background-image: url('<?php echo esc_url( $image_url ); ?>'); --jltpmd-popup-color: <?php echo esc_attr( $popup->get_content( 'btn_color' ) ); ?>;">
				...
				<div class="jltpmd-popup-modal-footer">
					...
					<div class="jltpmd-popup-countdown" style="display: none;">
						<span class="jltpmd-popup-countdown-text"><?php echo esc_html__( 'Deal Ends In', 'post-modified-date' ); ?></span>
						<div class="jltpmd-popup-countdown-time">
							<div>
								<span data-counter="days">00</span>
								<span><?php echo esc_html__( 'Days', 'post-modified-date' ); ?></span>
							</div>
							<span>:</span>
							<div>
								<span data-counter="hours">00</span>
								<span><?php echo esc_html__( 'Hours', 'post-modified-date' ); ?></span>
							</div>
							<span>:</span>
							<div>
								<span data-counter="minutes">00</span>
								<span><?php echo esc_html__( 'Minutes', 'post-modified-date' ); ?></span>
							</div>
							<span>:</span>
							<div>
								<span data-counter="seconds">00</span>
								<span><?php echo esc_html__( 'Seconds', 'post-modified-date' ); ?></span>
							</div>
						</div>
					</div>

					<!-- button  -->
					<a class="jltpmd-popup-button" target="_blank" href="<?php echo esc_url( $popup->get_content( 'button_url' ) ); ?>"><?php echo esc_html( $popup->get_content( 'button_text' ) ); ?></a>
				</div>
			</div>
		</div>

		<script>
			function jltpmd_popup_action(evt, $this, $action_type) {

				evt.preventDefault();

				$this.closest('.jltpmd-popup').fadeOut(200);
			...
			// Notice Dismiss
			jQuery('body').on('click', '.jltpmd-popup .popup-dismiss', function(evt) {
				jltpmd_popup_action(evt, jQuery(this), 'dismiss');
			});

			// Notice Disable
			jQuery('body').on('click', '.jltpmd-popup .popup-disable', function(evt) {
				jltpmd_popup_action(evt, jQuery(this), 'disable');
			});
		</script>
